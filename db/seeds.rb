...
if Channel.where(name: 'Super Fun Sample Test Channel').blank?
  Channel.create!(name: 'Super Fun Sample Test Channel', vimeo_id: 1129301)
end

if Video.where(title: 'Nickelodeon\'s Doug (opening credits)').blank?
  cid = Channel.where(vimeo_id: 1129301).first.id
  Video.create!(vimeo_id: 150707411, title: 'Nickelodeon\'s Doug (opening credits)', channel_id: cid)
end
